/**
 * PURPOSE: Chapter detail page — tabbed view of facts, sites, timeline, glossary, exam intel, exercises
 * USED BY: App Router (/chapters/[slug])
...
export default function ChapterDetailPage() {
  ...
  useEffect(() => {
    ...
    const loaders: Record<TabKey, () => Promise<void>> = {
      facts: async () => {
        ...
        const data = await api.getChapterFacts(slug);
        setFacts(data);
      },
      sites: async () => {
        if (sites.length > 0) return;
        const data = await api.getChapterSites(slug);
        setSites(data);
      },
      timeline: async () => {
        if (timeline.length > 0) return;
        const data = await api.getChapterTimeline(slug);
        setTimeline(data);
      },
      glossary: async () => {
        if (glossary.length > 0) return;
        const data = await api.getChapterTerms(slug);
        setGlossary(data);
      },
      "exam-intel": async () => {
        if (examIntel.length > 0) return;
        const data = await api.getChapterExamIntel(slug);
        setExamIntel(data);
      },
      exercises: async () => {
        if (exercises.length > 0) return;
        const data = await api.getChapterExercises(slug);
        setExercises(data);
      },
    };
  ...
  }, [activeTab, slug, facts.length, sites.length, timeline.length, glossary.length, examIntel.length, exercises.length]);
      ...
      <>
        ...
        <main className="min-h-screen flex items-center justify-center" style={{ background: "#FAF8F5", color: "#1A1A1A" }}>
          <div className="text-center">
            ...
            <a href="/chapters" className="text-sm underline" style={{ color: "#E07020" }}>
              ← Back to chapters
            </a>
          </div>
        </main>
  ...
  return (
    <>
      ...
      <main className="min-h-screen" style={{ background: "#FAF8F5", color: "#1A1A1A" }}>
        {/* ── Chapter header ── */}
        <section className="py-8 px-4 border-b" style={{ borderColor: "#E8E4DC" }}>
          <div className="max-w-4xl mx-auto">
            <a href="/chapters" className="text-xs mb-2 inline-block" style={{ color: "#E07020" }}>
              ← Back to chapters
            </a>
            <div className="flex items-center gap-3 mb-1">
              ...
              <span className="text-xs" style={{ color: "#7A7168" }}>
                {chapter.part_name} → {chapter.unit_name}
              </span>
            </div>
...
function SitesTab({ sites }: { sites: SiteItem[] }) {
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm mb-4" style={{ color: "#7A7168" }}>
        {sites.length} archaeological sites
      </p>
      {sites.map((s) => (
        <div
          key={s.id}
        ...
        >
          ...
          {s.state_region && (
            <p className="text-xs mb-1" style={{ color: "#E07020" }}>
              {s.state_region}
            </p>
          )}
          ...
          <p className="text-xs mt-2" style={{ color: "#B5AEA4" }}>
            {s.citation}
          </p>
        </div>
      ))}
  ...
  return (
    <div className="space-y-0">
      <p className="text-sm mb-6" style={{ color: "#7A7168" }}>
        {events.length} key dates and events
      </p>
      <div className="relative pl-6 border-l-2" style={{ borderColor: "#E07020" }}>
        {events.map((e) => (
          <div key={e.id} className="mb-6 relative">
            <span
              className="absolute -left-[33px] w-4 h-4 rounded-full border-2"
              style={{ background: "#FAF8F5", borderColor: "#E07020" }}
            />
            <span className="text-xs font-bold" style={{ color: "#E07020" }}>
              {e.date_text}
            </span>
            <p className="text-sm mt-1">{e.event}</p>
            <p className="text-xs mt-1" style={{ color: "#B5AEA4" }}>
              {e.citation}
            </p>
          </div>
        ))}
...
function ExamIntelTab({ entries }: { entries: ExamIntelEntry[] }) {
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm" style={{ color: "#7A7168" }}>
        {entries.length} exam insights
      </p>
      {Object.entries(byCategory).map(([cat, items]) => (
        <div key={cat}>
          <h3
            className="font-bold text-sm uppercase tracking-wide mb-3 pb-1 border-b"
            style={{ borderColor: "#E8E4DC", color: "#7A7168" }}
          >
            {cat}
          </h3>
          <div className="space-y-2">
...
function ExercisesTab({ exercises }: { exercises: Exercise[] }) {
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm mb-4" style={{ color: "#7A7168" }}>
        {exercises.length} practice exercises
      </p>
      {exercises.map((e, i) => (
        <div
          key={e.id}
